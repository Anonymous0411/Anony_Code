...
with open('patient_memory_train.json', 'r', encoding='utf-8') as file:
...
with open('CASE_HUB/patient_condiction_trainset_embedding.json', 'r', encoding='utf-8') as file:
  patient_condiction_embedding = json.load(file)

with open('patient_condiction_train.json', 'r', encoding='utf-8') as file:
...
def execute_DAG_task1(patient_key, DAG_dict, history_dialogue, history_diagnosis, second_room_list, train_executor=False):
  ...
  for idx, single_node in enumerate(DAG_dict['nodes']):
    ...
    if single_node['node_id'] == 'final_answer':
      ...
      final_answer_prompt = final_answer_ori.replace("{dialogue_record}", dialogue_record).replace("{tools_result}", tools_result).replace("{diagnosis}", solve_result).replace("{history_dialogue}", history_dialogue).replace("{history_diagnosis}", history_diagnosis).replace("{second_room_list}", second_room_list).replace("{patient_des}", patient_condiction[patient_key])

      # final_answer_prompt = final_answer_ori.replace("{dialogue_record}", dialogue_record).replace("{tools_result}", tools_result).replace("{diagnosis}", solve_result).replace("{patient_des}", patient_condiction[patient_key])
      if train_executor:
