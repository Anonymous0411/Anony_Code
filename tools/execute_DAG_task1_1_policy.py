...
def process_single_node(idx, single_node, history, single_patient_condiction, patient_key):
    if single_node.get('question') == 'random':
      return idx, '......'
    if single_node.get('question') != '无':
        question = single_node['question']
        response = patient_response(history, single_patient_condiction, question, patient_key=patient_key, stage="task1_level1")
        return idx, response
    return idx, None
...
def execute_DAG_task1_1_policy(patient_key, DAG_dict, train_executor=False, inference_con=False, node_weight=False):
  ...
  for idx, single_node in enumerate(DAG_dict['nodes']):
    ...
    if single_node['tools'] != '无':
      if "病例检索" in single_node['tools']:
        ...
        # # clinical lab
        # top3_case_key = patient_condiction_embedding[patient_key][:3]
        # re_case_1_desc = patient_condiction[top3_case_key[0]]
        # re_case_1_first = MedChain_data_gt[top3_case_key[0]]['task1-DG']
        # re_case_2_desc = patient_condiction[top3_case_key[1]]
        # re_case_2_first = MedChain_data_gt[top3_case_key[1]]['task1-DG']
        # re_case_3_desc = patient_condiction[top3_case_key[2]]
        # re_case_3_first = MedChain_data_gt[top3_case_key[2]]['task1-DG']


        DAG_dict['nodes'][idx]['result_tools'] = f'使用工具：病例检索\n过往相似病例：\n1、病例描述："{re_case_1_desc}"\n所属科室：{re_case_1_first}\n2、病例描述："{re_case_2_desc}"\n所属科室：{re_case_2_first}\n3、病例描述："{re_case_3_desc}"\n所属科室：{re_case_3_first}\n'
        break
    ...
    if single_node['node_id'] == 'final_answer':
      for single_node_child in DAG_dict['nodes']:
        ...
        if single_node_child['result_solve'] != 'pending':
          solve_result += single_node_child['result_solve']
      if node_weight == True:
        final_answer_prompt = final_answer_ori.replace("{dialogue_record}", dialogue_record).replace("{tools_result}", '无').replace("{diagnosis}", '无').replace("{description}", '无')
      else:
        final_answer_prompt = final_answer_ori.replace("{dialogue_record}", dialogue_record).replace("{tools_result}", tools_result).replace("{diagnosis}", solve_result).replace("{description}", patient_condiction[patient_key])
