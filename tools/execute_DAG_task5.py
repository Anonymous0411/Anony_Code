...
def process_single_node(idx, single_node, history, single_patient_condiction, patient_key):
    if single_node.get('question') != '无':
        ...
        response = patient_response(history, single_patient_condiction, question, patient_key=patient_key, stage="task5_treatment")
        return idx, response
    return idx, None
...
def execute_DAG_task5(patient_key, DAG_dict, pre_data, train_executor=False, inference_con=False, node_weight=False, run_model='train'):
  ...
  if run_model == 'train':
    ...
    with open('prompt/excute/task5.txt', 'r', encoding='utf-8') as file:
      final_answer_ori = file.read()

    with open('CASE_HUB/patient_condiction_trainset_embedding_task5.json', 'r', encoding='utf-8') as file:
    ...
    with open('CASE_HUB/patient_condiction_testset_embedding_task5.json', 'r', encoding='utf-8') as file:
      patient_condiction_embedding = json.load(file)
  ...
  for idx, single_node in enumerate(DAG_dict['nodes']):
    ...
    if single_node['node_id'] == 'final_answer':
      ...
      if node_weight == True:
        final_answer_prompt = final_answer_ori.replace("{dialogue_record}", dialogue_record).replace("{tools_result}", '无').replace("{diagnosis}", '无').replace("{pre_data}", pre_data).replace("{description}", patient_condiction[patient_key])
      else:
        final_answer_prompt = final_answer_ori.replace("{dialogue_record}", dialogue_record).replace("{tools_result}", tools_result).replace("{diagnosis}", solve_result).replace("{pre_data}", pre_data).replace("{description}", patient_condiction[patient_key])
