...
# with open('prompt/excute/task2_physical.txt', 'r', encoding='utf-8') as file:
#   final_answer_ori_phy = file.read()

# with open('prompt/excute/task2_asis.txt', 'r', encoding='utf-8') as file:
#   final_answer_ori_ass = file.read()

# with open('CASE_HUB/patient_condiction_trainset_embedding_task2.json', 'r', encoding='utf-8') as file:
...
with open('CASE_HUB/patient_condiction_testset_embedding_task2.json', 'r', encoding='utf-8') as file:
  patient_condiction_embedding = json.load(file)
...
def process_single_node(idx, single_node, history, single_patient_condiction, patient_key):
    if single_node.get('question') != '无':
        ...
        response = patient_response(history, single_patient_condiction, question, patient_key=patient_key, stage="task2_examination")
        return idx, response
    return idx, None
...
def execute_DAG_task2(patient_key, DAG_dict, train_executor=False, inference_con=False, node_weight=False):
  ...
  for idx, single_node in enumerate(DAG_dict['nodes']):
      ...
      if "病例检索" in single_node['tools'] and tool_flag == False:
        ...
        if len(top3_case_key) == 0:
          ...
          for single_rag_key_id, single_rag_key in enumerate(top3_case_key):
            ...
            if MedChain_data_gt[single_rag_key]['physical'] == '无':
              re_case_physical = '无'
            else:
              re_case_physical = str(list(MedChain_data_gt[single_rag_key]['physical'].keys()))

            if MedChain_data_gt[single_rag_key]['auxiliary'] == '无':
              re_case_auxiliary = '无'
            else:
              re_case_auxiliary = str(list(MedChain_data_gt[single_rag_key]['auxiliary'].keys()))

            # tools_desc += f"{single_rag_key_id+1}、病例描述：{re_case_desc}\n体格检查：{re_case_physical}\n辅助检查：{re_case_auxiliary}\n"
            tools_desc += f"{single_rag_key_id+1}、病例描述：{re_case_desc}\n检查：{re_case_physical}\n"
    ...
    if single_node['node_id'] == 'final_answer':
      ...
      if node_weight == True:
        final_answer_prompt_phy = final_answer_ori_phy.replace("{dialogue_record}", dialogue_record).replace("{tools_result}", '无').replace("{diagnosis}", '无').replace("{patient_des}", patient_condiction[patient_key])
        final_answer_prompt_ass = final_answer_ori_ass.replace("{dialogue_record}", dialogue_record).replace("{tools_result}", '无').replace("{diagnosis}", '无').replace("{patient_des}", patient_condiction[patient_key])
      else:
        final_answer_prompt_phy = final_answer_ori_phy.replace("{dialogue_record}", dialogue_record).replace("{tools_result}", tools_result).replace("{diagnosis}", solve_result).replace("{patient_des}", patient_condiction[patient_key])
        final_answer_prompt_ass = final_answer_ori_ass.replace("{dialogue_record}", dialogue_record).replace("{tools_result}", tools_result).replace("{diagnosis}", solve_result).replace("{patient_des}", patient_condiction[patient_key])

      # print(final_answer_prompt_phy)
      # print('-------'*50)
      # print(final_answer_prompt_ass)
      
      if train_executor:
        return final_answer_prompt_phy, final_answer_prompt_ass
